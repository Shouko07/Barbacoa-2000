(function(window) {
  ...
  function HTMLMagnifier(options) {
    ...
    function syncViewport() {
      ...
      const top = -y1 * _this.options.zoom - y2 * _this.options.zoom+50;
...
function prepareContent() {
  ...
  // Reinicializar DateDreamer en el clon
  const calendarOriginal = document.getElementById('calendar');
  const calendarCopy = bodyCopy.querySelector('#calendar');
  if (calendarOriginal && calendarCopy && window.DateDreamer) {
    // Usa las opciones originales si las tienes guardadas
    const options = calendarOriginal._dateDreamerOptions || {};
    new window.DateDreamer(calendarCopy, options);
  }
}
